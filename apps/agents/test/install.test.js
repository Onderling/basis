 ...
 *      override installs a non-catalogue card; uninstall reuses 's
 *      purge/revoke.
...
import { createStubCatalogue, STUB_CATALOGUE_CARDS } from '../src/defaultCatalogue.js';
...
const CARD = Object.freeze({
  ...
  'x-onderling': { id: 'catalogue:summariser', pubKey: 'pub-cat-summariser', role: 'service' },
});
...
describe('agents — P3 installAgent (install act + capability-security semantics)', () => {
  it('curated install grants ONLY the requested-and-declared subset (default-deny)', async () => {
    ...
    const catalogue  = createStubCatalogue([CARD]);

    const res = await installAgent({ registry, tokens, catalogue }, {
    ...
    expect(res.agentId).toBe('catalogue:summariser');
  ...
  it('install with NO grants is INERT — registered but zero authority', async () => {
    ...
    const res = await installAgent({ registry, catalogue }, { catalogueId: 'catalogue:summariser' });
  ...
  it('power-user override installs a NON-catalogue card (bypasses the catalogue)', async () => {
    const registry = buildRegistry();
    const tokens   = makeMockTokens();
    // NOTE: no catalogue source at all — the override path must not need one.
  ...
  it('honest structured errors: missing identity, missing source, unknown entry', async () => {
    ...
    // catalogueId but no catalogue source injected.
    expect((await installAgent({ registry }, { catalogueId: 'x' })).error).toBe('no-catalogue');
    // catalogueId not present in the source.
    const catalogue = createStubCatalogue([CARD]);
    expect((await installAgent({ registry, catalogue }, { catalogueId: 'nope' })).error)
      .toBe('catalogue-entry-not-found');
    // neither card nor catalogueId.
    expect((await installAgent({ registry }, {})).error).toBe('card-or-catalogueId-required');
  });
  ...
  it('uninstall reuses P2: purge hard-deletes, revoke soft-disables', async () => {
    ...
    const tokens   = makeMockTokens();
    const catalogue  = createStubCatalogue([CARD]);
    await installAgent({ registry, tokens, catalogue }, {
      catalogueId: 'catalogue:summariser', grants: ['summarise.thread'],
    });

    // Soft uninstall (revoke) — sweeps the grant token, keeps the entry.
    const rev = await revokeAgent({ registry, tokens }, { agentId: 'catalogue:summariser' });
    expect(rev.revoked).toBe(true);
    expect(rev.tokensRevoked).toBe(1);
    expect((await registry.lookup('catalogue:summariser')).revokedAt).not.toBeNull();

    // Hard uninstall (purge) — entry gone entirely.
    const pur = await purgeAgent({ registry, tokens }, { agentId: 'catalogue:summariser' });
    expect(pur.purged).toBe(true);
    expect(await registry.lookup('catalogue:summariser')).toBeNull();
  });
});

/* ── 2. Pluggable catalogue source + governance seam ─────────────────────── */
describe('agents — P3 listCatalogue (pluggable source; governance stubbed)', () => {
  it('no source → the honest "coming with the community catalogue" state', async () => {
    const res = await listCatalogue({ registry: buildRegistry() }, {});
    expect(res.ok).toBe(false);
    expect(res.error).toBe('no-catalogue');
    expect(typeof res.message).toBe('string');
    expect(res.catalogue).toEqual([]);
  });

  it('with the default stub source → installable cards with declared skills', async () => {
    const catalogue = createStubCatalogue();   // the shipped placeholder set
    const res = await listCatalogue({ registry: buildRegistry(), catalogue }, {});
    expect(res.ok).toBe(true);
    expect(res.count).toBe(STUB_CATALOGUE_CARDS.length);
    const summ = res.catalogue.find((c) => c.id === 'catalogue:summariser');
  ...
  it('a card from the catalogue is directly installable by the same act', async () => {
    const registry = buildRegistry();
    const catalogue  = createStubCatalogue();
    const list = await listCatalogue({ registry, catalogue }, {});
    const first = list.catalogue[0];
    const res = await installAgent({ registry, catalogue }, { catalogueId: first.id });
    expect(res.ok).toBe(true);
...
describe('agents — P3 capability-security: an installed agent can ONLY do what it was granted', () => {
  ...
  it('grants ONLY the granted skill; the same token is DENIED for an ungranted skill', async () => {
    ...
    const res = await installAgent(
      { registry, tokens: world.tokens, catalogue },
      { catalogueId: 'catalogue:summariser', grants: [GRANTED_SKILL] },
    );
  ...
  it('revoke (reuse P2 revokeAgent) makes even the granted skill go inert', async () => {
    ...
    const rev = await revokeAgent({ registry, tokens: world.tokens }, { agentId: 'catalogue:summariser' });
    expect(rev.tokensRevoked).toBe(1);
