 ...
 * capability ops land in the mobile catalogue), dispatches through the SAME `createRealHouseholdAgent` (1d's
...
const catalogue    = composeManifests();                       // the real mobile dispatch catalogue
...
describe('§1b mobile parity — generic `note` via the shared pipeline', () => {
  it('the MOBILE catalogue carries the synthetic generic op + its /add-note slash', () => {
    const entry = catalogue.opsById.get(genericAdd);
    ...
    expect(catalogue.opsById.has('addItem')).toBe(true);       // real ops not shadowed by the synth pass
  });

  it('the mobile parser resolves `/add-note …` → the generic op-id (same waist as web)', () => {
    const d = resolveDispatch(parseInput('/add-note buy stamps', catalogue, { threadId: 't' }), catalogue);
  ...
  it('the mobile gate (gateEntry.op → checkCapability) authorises when household is on, denies when off', () => {
    const op  = catalogue.opsById.get(genericAdd).op;          // exactly what CircleLauncherScreen passes
