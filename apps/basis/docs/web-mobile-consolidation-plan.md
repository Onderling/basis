# basis — web↔mobile consolidation plan (remove the divergence)
...
- **Most substrates ARE shared** — ~78 modules re-exported via the `@onderling-app/basis` barrel and
...
Guardrails for EVERY phase: full basis vitest green (currently 2282); web build (`vite build`)
...
Web (`apps/basis/locales/{en,nl}.json`, loaded by `src/localisation.js`) and mobile
(`apps/basis-mobile/locales/`, loaded by `core/localisation.js`) are separate bundles, but the
...
modules, so the next change can't drift between platforms. This is the basis instance of the
