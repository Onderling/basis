/**
 * basis v2 — circle "kind" factory family (policy · rules · recipe).
 *
 * ── Why this file exists ────────────────────────────────────────────────
 * The three circle broadcast "kinds" — policy (γ-next.policy), rules
 ...
 * LRU dedup, same on-disk key prefixes (`cc.circle<Kind>Pending.<id>`),
 ...
 * flat-doc shape (`makeCircleFlatDocConflict`), but the RECIPE conflict
...
const DEFAULT_DEDUP_CAP = param({ key: 'circleKind.dedupCap', scope: PARAM_SCOPE.DEVICE, kind: PARAM_KIND.INTERNAL, default: 256 });
...
/**
 * Build a circle-<kind>-broadcast peer-handler *factory* for one kind.
 *
 * The returned factory has the exact signature the three per-kind
 * receivers historically exposed (`makeCircle<Kind>PeerHandler({ ... })`).
 ...
 */
export function makeCircleKindReceiver({ subtype, payloadKey, logTag }) {
  return function makeCirclePeerHandler({
    pendingStore,
    dedup    = null,
    logger   = console,
    ...
    if (!pendingStore || typeof pendingStore.set !== 'function') {
      throw new Error(`${logTag} makeCirclePeerHandler: pendingStore.set required`);
    }
    const seen = dedup ?? new LruSet(dedupCap);

    return async function onCircleBroadcast(fromPeerAddr, payload) {
...
// Tiny LRU set — one shared copy for all circle kinds. Exported so any other
// per-circle broadcast receiver (e.g. the roster "pull-me" signal in
// the silent system lane) reuses THIS dedup rather than growing a second copy.
export class LruSet {
...
/**
 * Build a per-circle pending-<kind> store from injected IO.  Stashes ONE
 ...
 */
export function createCircleKindPendingStore({ load, save, remove } = {}) {
  return {
    async get(circleId) {
 ...
 *
 * @param {string} keyPrefix  e.g. 'cc.circlePolicyPending.'
 * @param {Storage} [storage] window.localStorage or an injected shim
 * @returns {{ load: Function, save: Function, remove: Function }}
 */
export function makeCircleKindPendingLocalIo(keyPrefix, storage = globalThis.localStorage) {
  return {
    load: async (circleId) => {
      try {
...
export function makeCircleFlatDocConflict({ deepIncoming = false } = {}) {
  /**
