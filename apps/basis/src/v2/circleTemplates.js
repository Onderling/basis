...
/** @type {Record<string, CircleTemplate>} */
export const CIRCLE_TEMPLATES = Object.freeze({
 ...
 *
 * It sits BESIDE `CIRCLE_TEMPLATES` rather than inside it, for the same reason `TEMPLATE_CONVERSATION_KINDS`
...
export const CIRCLE_KINDS = Object.freeze(
  Object.keys(CIRCLE_TEMPLATES).filter((k) => k !== '_default'),
);
 ...
 * @returns {CircleTemplate}
 */
export function defaultsForKind(kind) {
  if (typeof kind === 'string' && Object.prototype.hasOwnProperty.call(CIRCLE_TEMPLATES, kind) && kind !== '_default') {
    return CIRCLE_TEMPLATES[kind];
  }
  return CIRCLE_TEMPLATES._default;
}
