 ...
 * @param {Function} [a.identityOf]      who the poster IS — a post carries whatever address its author
 *                                       spoke from, and a thread is keyed by the person (Frits
 *                                       2026-09-03), so the reply lands in the thread you already have
 *                                       with them instead of opening a second one
 ...
 */
import { contactIdFor } from '../core/handlers/threadedChat.js';

export async function replyToPost({ callSkill, contactChannel = null, notePeer = null, identityOf = null, itemId, body } = {}) {
  ...
  const addr = typeof r.toPubKey === 'string' && r.toPubKey ? r.toPubKey : null;
  const toPubKey = addr ? contactIdFor(identityOf, addr) : null;
  ...
  // no row to reach it by afterwards. Walked on a phone, where the reply landed and then vanished.
