 ...
 *  1. `/settings` typed in the circle composer dispatches as a BUILT-IN (G17) — it opens the
...
async function openCircleComposer(page, name) {
...
test('/settings (G17 built-in) opens the settings panel with the Connection controls; private-DM greys out under pod-only', async ({ page }) => {
  ...
  await openCircleComposer(page, 'A3 Settings Circle');
