 ...
 *
 * v2 DOM conventions: circle composer `.circle-view__composer-input` / `.circle-view__composer-send`,
 * bubbles `.circle-view__bubble`, launcher `.circle-tile` / `.circle-launcher__new`, circles tab
 * `[data-tab="circles"]`, chat toggle `.circle-view__view-toggle-btn`.
 */
import { expect } from '@playwright/test';

/** Boot the v2 app and open a circle chat composer. Resolves once `.circle-view__composer-input` is visible.
 *  Lifted from the per-spec `openCircleComposer` (circle-view-*.spec.js) so migrated specs share ONE boot. */
/**
...
export async function circleBubbles(page) {
  return page.locator('.circle-view__bubble').allTextContents();
}
