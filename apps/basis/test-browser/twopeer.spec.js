...
// hidden, so leave via the "← kringen" back button first.
async function gotoKringen(page) {
...
test('two peers pair over the app transport + collaborate', async ({ browser }) => {
  ...
  await gotoKringen(a); await openCircleMatching(a, /peer.?circle/i); await toChat(a);
  await gotoKringen(b); await openCircleMatching(b, /peer.?circle/i); await toChat(b);
  ...
  try {
    ...
    const bTabs = b.locator('.circle-kring__tab');
    const bTabLabels = (await bTabs.allTextContents()).map((s) => s.trim());
    console.log('B kring tabs:', JSON.stringify(bTabLabels));
    ...
    if (takenIdx >= 0) {
      ...
      for (let i = 0; i < 6 && !seen; i++) {
        const rows = b.locator('.circle-kring__task');
  ...
  try {
    await toChat(a);
    const aTabs = a.locator('.circle-kring__tab');
    ...
    let entrust = a.locator('.circle-kring__bubble-action--mandate, .circle-kring__task-action--mandate, [data-action="mandate"]');
    if (!(await entrust.count())) {
      entrust = a.locator('.circle-kring__task button, .circle-kring__task-action, .circle-kring__bubble-action').filter({ hasText: /toevertrouwen/i });
    }
