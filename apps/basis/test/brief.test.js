...
describe('runBrief — fan-out', () => {
  it("fans across declared apps + returns ordered sections", async () => {
    ...
    const r = await runBrief({ catalogue, callSkill });
  ...
  it("rejects null catalogue / non-function callSkill", async () => {
    await expect(runBrief({ catalogue: null, callSkill: () => {} }))
      .rejects.toThrow(/catalogue/);
    await expect(runBrief({ catalogue: makeCatalogue([]), callSkill: null }))
      .rejects.toThrow(/callSkill/);
  });

  it("returns a cacheKey + generatedAt timestamp", async () => {
    const r = await runBrief({
      catalogue: makeCatalogue([{ opId: 'a', appOrigin: 'a', summarySkill: 's' }]),
...
describe('runBrief — cache interaction', () => {
  it("uses cached value when present + not bypassed", async () => {
    ...
    const catalogue = makeCatalogue([{ opId: 'a', appOrigin: 'a', summarySkill: 's' }]);
    await runBrief({ catalogue, callSkill, cache });
    await runBrief({ catalogue, callSkill, cache });
  ...
  it("bypassCache: true re-runs", async () => {
    ...
    const catalogue = makeCatalogue([{ opId: 'a', appOrigin: 'a', summarySkill: 's' }]);
    await runBrief({ catalogue, callSkill, cache });
    await runBrief({ catalogue, callSkill, cache, bypassCache: true });
