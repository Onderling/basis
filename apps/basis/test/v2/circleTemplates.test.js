/**
 * basis v2 — circleTemplates substrate tests (β.4).
 *
 * Covers:
...
import {
  CIRCLE_TEMPLATES, CIRCLE_KINDS, defaultsForKind, applyTemplate, markAxisTouched, unknownKeysFor,
...
describe('defaultsForKind', () => {
  it('returns the household template for kind="household"', () => {
    expect(defaultsForKind('household')).toBe(CIRCLE_TEMPLATES.household);
  });

  it('returns _default for an unknown kind', () => {
    expect(defaultsForKind('unknown-kind')).toBe(CIRCLE_TEMPLATES._default);
  });

  it('returns _default for null/undefined/non-string', () => {
    expect(defaultsForKind(null)).toBe(CIRCLE_TEMPLATES._default);
    expect(defaultsForKind(undefined)).toBe(CIRCLE_TEMPLATES._default);
    expect(defaultsForKind(42)).toBe(CIRCLE_TEMPLATES._default);
  });
  ...
  it('refuses to expose `_default` directly via its key', () => {
    ...
    expect(defaultsForKind('_default')).toBe(CIRCLE_TEMPLATES._default);
    expect(CIRCLE_KINDS).not.toContain('_default');
  });
...
describe('applyTemplate — empty state', () => {
  it('fills features + axes from the household template', () => {
    ...
    expect(next.features).toEqual(CIRCLE_TEMPLATES.household.features);
  ...
  it('falls back to _default for an unknown kind', () => {
    ...
    expect(next.revealPolicy).toBe(CIRCLE_TEMPLATES._default.revealPolicy);
    expect(next.pod).toBe(CIRCLE_TEMPLATES._default.pod);
  });
...
describe('applyTemplate — the USER\'s choices win, and only theirs (decision 4, 2026-07-29)', () => {
  ...
  it('…while a choice made along the way still survives the switch', () => {
    ...
    const fresh = applyTemplate({}, 'friends');
...
describe('template shape — keys match circlePolicy.js', () => {
  it('every template uses only known top-level keys + known features + valid enums', () => {
    for (const [name, tpl] of Object.entries(CIRCLE_TEMPLATES)) {
  ...
  it('round-trips every key for the four known kinds + _default', () => {
    for (const name of [...CIRCLE_KINDS, '_default']) {
      const tpl = CIRCLE_TEMPLATES[name];
