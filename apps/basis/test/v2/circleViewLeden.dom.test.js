...
//
// G16 — the real LEDEN (members) tab: the circle view lists the circle's trail-roster
...
describe('renderCircleView · LEDEN tab', () => {
  it('renders one tappable row per roster member (not the tab-coming placeholder)', () => {
    const el = mount();
    renderCircleView(el, { circle, rows: [], tabs, activeTab: 'leden', members, selfWebid: 'me', t });

    expect(el.querySelector('.circle-circle__placeholder')).toBeNull();
    const rows = el.querySelectorAll('.circle-circle__member');
    expect(rows.length).toBe(2);
    expect(rows[0].dataset.memberId).toBe('me');
    expect(rows[0].querySelector('.circle-circle__member-primary').textContent).toBe('@Owl');
    expect(rows[0].querySelector('.circle-circle__member-secondary').textContent).toBe('Frits');
  });

  it('badges the viewer\'s own row', () => {
    const el = mount();
    renderCircleView(el, { circle, rows: [], tabs, activeTab: 'leden', members, selfWebid: 'me', t });
    const mine = el.querySelector('.circle-circle__member--self');
    expect(mine).not.toBeNull();
    expect(mine.dataset.memberId).toBe('me');
    expect(mine.querySelector('.circle-circle__member-you').textContent).toBe('circle.leden_tab.you');
    // the other member's row is not badged.
    const others = [...el.querySelectorAll('.circle-circle__member')].filter((r) => !r.classList.contains('circle-circle__member--self'));
    expect(others).toHaveLength(1);
    expect(others[0].querySelector('.circle-circle__member-you')).toBeNull();
  });

  it('a member-row tap reaches the host with the tapped member', () => {
    const el = mount();
    const onMemberTap = vi.fn();
    renderCircleView(el, { circle, rows: [], tabs, activeTab: 'leden', members, selfWebid: 'me', onMemberTap, t });
  ...
  it('shows a loading state when the roster is not loaded yet (members == null)', () => {
    const el = mount();
    renderCircleView(el, { circle, rows: [], tabs, activeTab: 'leden', members: null, t });
    expect(el.querySelector('.circle-circle__leden-loading').textContent).toBe('circle.leden_tab.loading');
    expect(el.querySelector('.circle-circle__member')).toBeNull();
  });

  it('shows an empty state when the roster loaded empty', () => {
    const el = mount();
    renderCircleView(el, { circle, rows: [], tabs, activeTab: 'leden', members: [], t });
    expect(el.querySelector('.circle-circle__leden-empty').textContent).toBe('circle.leden_tab.empty');
  });
});
