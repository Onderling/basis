...
describe('clarifyCommandTargets', () => {
  it('binds the id and returns ready on a UNIQUE match', async () => {
    ...
    const r = await clarifyCommandTargets({ opId: 'markComplete', args: { target: 'dishes' } }, { catalogue: CATALOGUE, lookup, scope: { id: 'circle-A' } });
  ...
  it('returns clarify with candidates on an AMBIGUOUS match', async () => {
    ...
    const r = await clarifyCommandTargets({ opId: 'markComplete', args: { target: 'dishes' } }, { catalogue: CATALOGUE, lookup });
  ...
  it('returns unresolved when a required id-param matches NOTHING', async () => {
    ...
    const r = await clarifyCommandTargets({ opId: 'markComplete', args: { target: 'laundry' } }, { catalogue: CATALOGUE, lookup });
  ...
  it('returns unresolved when a required id-param is missing entirely', async () => {
    const r = await clarifyCommandTargets({ opId: 'markComplete', args: {} }, { catalogue: CATALOGUE, lookup: lookupOf([]) });
  ...
  it('passes through commands with no id-like params (ready, no lookup)', async () => {
    const lookup = vi.fn();
    const r = await clarifyCommandTargets({ opId: 'addTask', args: { title: 'milk' } }, { catalogue: CATALOGUE, lookup });
  ...
  it('skips lookup when the value is already a concrete id', async () => {
    const lookup = vi.fn();
    const r = await clarifyCommandTargets({ opId: 'markComplete', args: { target: '01HXYZ1234567890ABCDEF' } }, { catalogue: CATALOGUE, lookup });
