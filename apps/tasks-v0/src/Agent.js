...
export async function createTasksAgent({
  ...
  if (shouldRegisterSkills) {
    wireSkills({
      ...
      members,
      // No host reader here on purpose. A single-circle agent is composed with an EXPLICIT `roles`
      // map — the caller stating who administers this circle — and without a host reader every
      // authority gate reads exactly that declared map, per circle (see `skills/roleOf.js`). A
      // multi-circle host injects `circleRoleOf` instead: the circle's folded roster, which is the
      // circle's answer rather than the device's. A reader scoped to ONE circle's map would answer
      // for every circle the dashboard enumerates, and that is the wrong source for the others.
    });
  }
