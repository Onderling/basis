 ...
 * ranking. These tests assert RESULTS (ranked catalogue membership + ORDER) over
...
import { issueEndorsement, createCatalogueSource, walkTrustGraph } from '../index.js';

/* ── Fixtures ──────────────────────────────────────────────────────────── */

async function makeIdentity() { return AgentIdentity.generate(new VaultMemory()); }

/** An installable AGENT card (default role 'service' → a catalogue candidate). */
...
describe('G2 — transitive reach (the walk)', () => {
  it('root → curator B → agent X: X is a candidate at depth 2; an agent behind an UNREACHABLE curator is excluded', async () => {
    ...
    const cat  = createCatalogueSource({ resolveEndorsements: g.resolveEndorsements, roots: [root.pubKey], resolveCard: g.resolveCard, maxDepth: 4 });
    const list = await cat.list();

    expect(idsOf(list)).toEqual(['catalogue:X']);            // X reachable; Y excluded; curator B not listed
    expect(list[0]['x-onderling'].endorsement.depth).toBe(2); // two endorsement hops from the root
    expect(await cat.get('catalogue:Y')).toBeNull();
  });
});
...
describe('G2 — bounded depth', () => {
  it('an agent reachable only beyond maxDepth is EXCLUDED', async () => {
    ...
    g.card(deep, agentCard(deep.pubKey, { id: 'catalogue:deep' }));
    ...
    const shallow = createCatalogueSource({ resolveEndorsements: g.resolveEndorsements, roots: [root.pubKey], resolveCard: g.resolveCard, maxDepth: 2 });
    expect(idsOf(await shallow.list())).toEqual([]);

    // maxDepth 3 → deep is reachable.
    const deeper = createCatalogueSource({ resolveEndorsements: g.resolveEndorsements, roots: [root.pubKey], resolveCard: g.resolveCard, maxDepth: 3 });
    expect(idsOf(await deeper.list())).toEqual(['catalogue:deep']);
  });
});
...
describe('G2 — trust-path-proximity ranking', () => {
  it('a directly-root-endorsed agent outranks a depth-3 one; breadth breaks ties', async () => {
    ...
    g.card(B, curatorCard(B.pubKey, { id: 'B' }));
    g.card(C, curatorCard(C.pubKey, { id: 'C' }));
    g.card(near, agentCard(near.pubKey, { id: 'catalogue:near' }));
    g.card(far,  agentCard(far.pubKey,  { id: 'catalogue:far' }));
    ...
    const cat  = createCatalogueSource({ resolveEndorsements: g.resolveEndorsements, roots: [root.pubKey], resolveCard: g.resolveCard, maxDepth: 4 });
    const list = await cat.list();
    expect(idsOf(list)).toEqual(['catalogue:near', 'catalogue:far']);   // proximity: near (1) before far (3)
  ...
  it('same depth → MORE distinct reachable endorsers ranks higher (breadth tiebreak)', async () => {
    ...
    g.card(popular, agentCard(popular.pubKey, { id: 'catalogue:popular' }));
    g.card(lonely,  agentCard(lonely.pubKey,  { id: 'catalogue:lonely' }));

    g.endorse(root1, popular);
    g.endorse(root2, popular);
    g.endorse(root1, lonely);

    const cat  = createCatalogueSource({ resolveEndorsements: g.resolveEndorsements, roots: [root1.pubKey, root2.pubKey], resolveCard: g.resolveCard, maxDepth: 4 });
    const list = await cat.list();
    expect(idsOf(list)).toEqual(['catalogue:popular', 'catalogue:lonely']);   // same depth 1; popular has 2 endorsers
...
describe('G2 — cycle safety', () => {
  it('an a ↔ b endorsement cycle terminates (no infinite loop)', async () => {
    ...
    g.card(b, curatorCard(b.pubKey, { id: 'B' }));
    g.card(leaf, agentCard(leaf.pubKey, { id: 'catalogue:leaf' }));
    ...
    const cat  = createCatalogueSource({ resolveEndorsements: g.resolveEndorsements, roots: [root.pubKey], resolveCard: g.resolveCard, maxDepth: 6 });
    const list = await cat.list();               // must return (the test completing IS the assertion)
    expect(idsOf(list)).toEqual(['catalogue:leaf']);
...
describe('G2 — multi-root union', () => {
  it("two roots' subgraphs union; an agent reachable from EITHER appears", async () => {
    ...
    g.card(onlyA, agentCard(onlyA.pubKey, { id: 'catalogue:onlyA' }));
    g.card(onlyB, agentCard(onlyB.pubKey, { id: 'catalogue:onlyB' }));

    g.endorse(rootA, onlyA);
    g.endorse(rootB, onlyB);

    // With only rootA, onlyB is invisible.
    const single = createCatalogueSource({ resolveEndorsements: g.resolveEndorsements, roots: [rootA.pubKey], resolveCard: g.resolveCard, maxDepth: 4 });
    expect(idsOf(await single.list())).toEqual(['catalogue:onlyA']);

    // Union of both roots.
    const both = createCatalogueSource({ resolveEndorsements: g.resolveEndorsements, roots: [rootA.pubKey, rootB.pubKey], resolveCard: g.resolveCard, maxDepth: 4 });
    expect(idsOf(await both.list()).sort()).toEqual(['catalogue:onlyA', 'catalogue:onlyB']);
  });
});
...
describe('G2 — flag precedence (a reachable curator flag excludes outright)', () => {
  it('a flag from any reachable curator removes an otherwise-recommended agent, even with a closer recommend', async () => {
    ...
    const B    = await makeIdentity();
    const x    = await makeIdentity();
    const cardX = agentCard(x.pubKey, { id: 'catalogue:x' });
    ...
    expect(await cat.get('catalogue:x')).toBeNull();
  });
});

/* ── OFFLINE CACHE ──────────────────────────────────────────────────────── */
describe('G2 — offline read-through cache', () => {
  it('serves the last good ranked catalogue when the endorsement source is unreachable', async () => {
    const g = graph();
    const root = await makeIdentity();
    const x = await makeIdentity();
    g.card(root, curatorCard(root.pubKey, { id: 'root' }));
    ...
    const cat = createCatalogueSource({ resolveEndorsements, roots: [root.pubKey], resolveCard: g.resolveCard, maxDepth: 4, cache });
    expect(idsOf(await cat.list())).toEqual(['catalogue:x']);   // online: derives + fills cache

    online = false;
    expect(idsOf(await cat.list())).toEqual(['catalogue:x']);   // offline: served from cache
    expect((await cat.get('catalogue:x'))['x-onderling'].id).toBe('catalogue:x');
  });

  it('with no cache, an unreachable source throws (no silent empty catalogue)', async () => {
    const root = await makeIdentity();
    const cat = createCatalogueSource({
...
describe('G2 — walkTrustGraph pure + endorsementResource pool back-compat', () => {
  it('the pool seam (a single shared list) reproduces the walk (G1 special case)', async () => {
    ...
    const cat = createCatalogueSource({ endorsementResource, roots: [root.pubKey], resolveCard: g.resolveCard, maxDepth: 4 });
    expect(idsOf(await cat.list())).toEqual(['catalogue:x']);   // same result as the resolver seam
  });
