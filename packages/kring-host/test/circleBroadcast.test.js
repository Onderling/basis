...
describe('circleChatMessageEvent', () => {
  it('builds the canonical circle chat-message event', () => {
    expect(circleChatMessageEvent({ msgId: 'm1', ts: 7, circleId: 'c', actor: 'me', text: 'hi' })).toEqual({
      id: 'm1', ts: 7, app: 'circle', type: 'chat-message', actor: 'me',
      payload: { circleId: 'c', text: 'hi', kind: 'chat-message' },
    });
  });
  it('includes buttons only when present', () => {
    const withBtns = circleChatMessageEvent({ msgId: 'm', ts: 1, circleId: 'c', actor: 'bot', text: 't', buttons: [{ id: 'a', label: 'A' }] });
